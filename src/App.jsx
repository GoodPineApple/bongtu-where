...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch(STORES_JSON_URL)
      .then((res) => {
        if (!res.ok) throw new Error(`판매소 데이터를 불러오지 못했습니다. (${res.status})`)
        return res.json()
      })
      .then((json) => {
        if (cancelled) return
  ...
  useEffect(() => {
    if (selectedStore && !visibleStores.some((s) => s.id === selectedStore.id)) {
  ...
  const handleReport = useCallback(
    (status) => {
      ...
      setStores((prev) =>
        prev.map((s) => (s.id === id ? { ...s, stockStatus: status, updatedAt: now } : s)),
      )
      setSelectedStore((prev) => (prev && prev.id === id ? { ...prev, stockStatus: status, updatedAt: now } : prev))
      const labels = { FULL: '재고 있음으로 반영했습니다.', FEW: '소량 남음으로 반영했습니다.', EMPTY: '품절로 반영했습니다.' }
  ...
  const showDataBanner =
    dataLoadError || dataErrors.length > 0 || dataLoading || (stores.length === 0 && !dataLoadError && !dataLoading)
    ...
    <div className="relative h-full w-full bg-slate-50">
      ...
      {showDataBanner && (
        <div className="pointer-events-none absolute left-3 right-3 top-28 z-10 max-h-32 overflow-y-auto rounded-lg bg-amber-50/95 px-3 py-2 text-xs text-amber-900 shadow ring-1 ring-amber-200">
          {dataLoading && <p className="pointer-events-auto">판매소 데이터를 불러오는 중입니다…</p>}
          {dataLoadError && <p className="pointer-events-auto">{dataLoadError}</p>}
          {!dataLoading &&
            !dataLoadError &&
          ...
          {!dataLoading && !dataLoadError && stores.length === 0 && (
            <p className="pointer-events-auto">
              <code className="rounded bg-amber-100 px-1">public/전국종량제봉투판매소표준데이터.json</code> 파일을
              확인하세요.
            </p>
          )}
        </div>
