...
/**
 * @param {{ map: object | null, mapReady: boolean }} props
 */
export default function MapZoomControls({ map, mapReady }) {
  const [, setLevelTick] = useState(0)

  useEffect(() => {
    if (!mapReady || !map || !window.kakao?.maps?.event) return undefined
    const handle = () => setLevelTick((n) => n + 1)
    window.kakao.maps.event.addListener(map, "zoom_changed", handle)
    return () => {
      window.kakao.maps.event.removeListener(map, "zoom_changed", handle)
    }
  }, [map, mapReady])
