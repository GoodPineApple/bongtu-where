...
export default function MapContainer({
  ...
  useEffect(() => {
    ...
    loadKakaoSdk(appKey)
      ...
      .catch((e) => {
        if (!cancelled) setSdkError(e?.message || '지도를 불러오지 못했습니다.')
      })
    ...
    return (
      <div className="flex h-full items-center justify-center bg-slate-100 px-4 text-center text-sm text-slate-600">
        <p>
          ...
          <code className="rounded bg-slate-200 px-1">VITE_KAKAO_MAP_APP_KEY</code>를 설정한 뒤 다시
          실행하세요.
        </p>
      </div>
    )
  ...
  return <div ref={containerRef} className="h-full w-full" role="application" aria-label="지도" />
}
