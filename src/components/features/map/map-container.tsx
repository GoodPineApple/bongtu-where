  ...
  return new Promise((resolve, reject) => {
    ...
    window.kakao.maps.load(() => {
      if (mapsReady()) resolve()
        ...
        reject(
          new Error(
            "카카오 지도 API가 준비되지 않았습니다. JavaScript 키와 [플랫폼]에 개발 도메인 등록을 확인하세요.",
          ),
        )
      }
    })
    ...
    kakaoSdkPromise = new Promise((resolve, reject) => {
      ...
      if (existing) {
        ...
        const onScriptOrReady = () => {
          runAfterKakaoMapsLoad().then(resolve).catch((e: Error) =>
            fail(e.message),
          )
        }
        if (window.kakao?.maps?.load) {
      ...
      script.onload = () => {
        runAfterKakaoMapsLoad().then(resolve).catch((e: Error) => fail(e.message))
      }
      script.onerror = () =>
        fail(
...
type KakaoMapHandle = {
  ...
  setCenter: (ll: object) => void
  panTo: (ll: object) => void
  getLevel: () => number
  setLevel: (level: number) => void
}

type KakaoOverlayHandle = {
  setMap: (map: KakaoMapHandle | null) => void
}

type OverlayCacheEntry = {
  overlay: KakaoOverlayHandle
...
export function MapContainer({
  ...
  const storesByIdRef = useRef(new Map<string, Store>())
  const overlaysCacheRef = useRef(new Map<string, OverlayCacheEntry>())
  ...
  const ensureOverlay = useCallback((store: Store) => {
  ...
  const recomputeVisible = useCallback(() => {
    ...
    if (inBounds.length > MAX_VISIBLE_OVERLAYS) {
      ...
      visibleSlice = inBounds.slice(0, MAX_VISIBLE_OVERLAYS)
    }

    const nextVisible = new Set<string>()
    for (const s of visibleSlice) nextVisible.add(s.id)
    ...
    for (const store of visibleSlice) {
  ...
  useEffect(() => {
    ...
    const incoming = new Map<string, Store>()
