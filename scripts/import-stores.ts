...
import { parseStoresStandardJson } from "../src/lib/data/parse-stores-standard-json"
import { storeToStoresTableRow } from "../src/lib/data/store-to-db-row"
...
if (!url || !serviceKey) {
  console.error(
    "대량 적재에 필요한 값:\n" +
      "  · Project URL → NEXT_PUBLIC_SUPABASE_URL (또는 Node 전용 SUPABASE_URL) 중 하나\n" +
      "  · SUPABASE_SERVICE_ROLE_KEY (대시보드 API의 service_role, 로컬·스크립트 전용)\n" +
      "프론트용 anon 키로는 RLS 때문에 upsert 할 수 없습니다.\n" +
      "service_role 은 Git/프론트(NEXT_PUBLIC_*)에 넣지 마세요.",
  )
  process.exit(1)
}
...
function dedupeRowsById(rows: Record<string, unknown>[]) {
